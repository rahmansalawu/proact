...
function ConsultantCard({ profile, metrics, onEngage, expanded = false }: { profile: MarketplaceRecord; metrics: ReturnType<typeof score>; onEngage: () => void; expanded?: boolean }) {
  ...
  return <article className={`consultant-card ${expanded ? "expanded" : ""}`}>
    <div className="consultant-card-top"><div className="consultant-avatar">{text(p.displayName).split(/\s+/).map((part) => part[0]).join("").slice(0, 2)}</div><div><div className="verified-name"><h3>{text(p.displayName) || profile.title}</h3>{Boolean(p.verified) && <CheckCircle2 size={14} aria-label="Verified profile" />}</div><p>{text(p.headline)}</p><span><MapPin size={12} /> {text(p.location) || "Location not provided"}</span></div><div className="trust-score"><strong>{metrics.trust}</strong><span>TRUST</span></div></div>
...
function EngagementCard({ record, saving, onEdit, onUpdate }: { record: MarketplaceRecord; saving: boolean; onEdit: () => void; onUpdate: (payload: Record<string, unknown>, status: string) => Promise<void> }) {
  ...
  return <article className="engagement-card">
    <div className="engagement-heading"><div><span>{record.reference}</span><h3>{record.title}</h3><p>{text(p.consultantName) || "Open brief"} · {text(p.client) || record.owner}</p></div><span className={`status-pill ${record.status === "Disputed" ? "red" : ""}`}><i />{record.status}</span></div>
...
function PostCard({ record, actorName, onUpdate }: { record: MarketplaceRecord; actorName: string; onUpdate: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <article className="community-post">
    <div className="post-author"><div>{text(p.author).split(/\s+/).map((part) => part[0]).join("").slice(0, 2)}</div><span><strong>{text(p.author)}</strong><small>{text(p.postType)} · {text(p.topic)}</small></span></div>
